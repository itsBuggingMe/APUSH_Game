...
namespace APUSH_Game.Helpers
{
    ...
    internal class Animation
    {
        ...
        /// </summary>
        public Animation(int delay, Action OnEnd) : this(Actions.Empty, 0, OnEnd, new KeyFrame(0, delay, AnimationType.None))
        {
        ...
        public Animation Reset(Action<float> modifier, float startingValue, Action OnEnd, params KeyFrame[] keyFrames)
        {
            if (keyFrames.Length == 0)
                throw new ArgumentException("Animation must have at least one keyframe", nameof(keyFrames));
